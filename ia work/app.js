...
async function syncFromUrl() {
  ...
  try {
    ...
    if (Array.isArray(remoteEntries)) { entries = remoteEntries; localStorage.setItem(STORAGE_KEY, JSON.stringify(entries)); render(); }
    setStatus('Sincronizado');
  } catch { setStatus('Sin conexión compartida'); }
}

async function pushToServer() {
  if (!SYNC_ENDPOINT) return;
  try {
    await fetch(SYNC_ENDPOINT, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(entries) });
